...
CMysqlServer::CMysqlServer()
  ...
  temp_buffer_ =
      reinterpret_cast<char*>(malloc(MAX_PACKET_SIZE * sizeof(char)));
...
int CMysqlServer::Start() {
  ...
  if (ListenPort(port_) == C_ERROR) {
    //		return C_ERROR;
    //		cout<<"The port of JDBC server is illegal.Please collect the
    //port
    // of
    // JDBC in .../CLAIMS/conf/config"<<endl;
...
bool CMysqlServer::AcceptConnection(int fd, int& connected_fd) {
  ...
  connected_fd =
      accept(fd, reinterpret_cast<sockaddr*>(&connected_socket_addr), &length);
  if (connected_fd < 0) {
    ...
    // connected_socket));
    fd_to_session_.insert(pair<int, CMysqlConnection*>(
        connected_fd,
        new CMysqlConnection(connected_fd, connected_socket_addr)));
  }
...
int CMysqlServer::CheckCommand(char* command) {
  ...
  if (request_.find("SHOW SESSION VARIABLES LIKE 'lower_case_table_names'",
                    0) != -1) {
    return 5;
  }
  return -1;
...
int CMysqlServer::DoComQuery(int fd, MysqlCommandPacket* packet) {
  ...
  if (temp > 0) {
    ...
    switch (temp) {
      ...
      case 3:
        command.cmd =
            "select Collation,Charset,I_d,D_efault,Compiled,Sortlen from "
            "collationA;";
        break;
      case 4:
      ...
      case 5:
        command.cmd =
            "select Variable_name, Valve from variables where Variable_name = "
  ...
  if (strcmp(q, version_query) == 0) {
    ...
    while(true){
            ...
            if(rs.result_got_[i] == true){
                    ...
                    //cout<<"mysql_server.cpp:379 now result is
:"<<buff_to_send<<endl;
                    ...
                    if(result_set != NULL){
                            ...
                            //cout<<"mysql_server.cpp:381 now result is
:"<<buff_to_send<<endl;
                            //cout<<"fd is:"<<i<<"command.socket_fd
is:"<<command.socket_fd<<endl;
...
//				else{
//					LOG(WARNING)<< "direct_execute failed,
ret is " << ret << endl;
//				}
...
int CMysqlServer::SendServerVersion(int fd) {
  ...
  ret = ProcessSinglePacket(fd, buffer, pos);
  if (ret != C_SUCCESS) {
    LOG(ERROR) << "process field packet failed,ret is " << ret << endl;
  }
  ...
  ret = ProcessSinglePacket(fd, buffer, pos);
  if (ret != C_SUCCESS) {
    LOG(ERROR) << "process eof packet failed,ret is " << ret << endl;
  }
  ...
  if ((ret = CMysqlUtil::store_length(buffer, len, length, pos)) == C_SUCCESS) {
    if (len - pos >= length) {
  ...
  ret = ProcessSinglePacket(fd, buffer, pos);
  if (ret != C_SUCCESS) {
    LOG(ERROR) << "process row packet failed,ret is " << ret << endl;
  }
...
int CMysqlServer::SendOkPacket(int fd, const MysqlResultSet& result,
                               uint16_t server_status,
                               MysqlCommandPacket* packet) {
...
int CMysqlServer::ProcessRowPackets(int fd, char* buffer, int64_t& pos,
  ...
  while (block = it.nextBlock()) {
    ...
    while (mysql_row.tuple_ = block_it->nextTuple()) {
      ...
      // break;
      if (ret != C_SUCCESS) {
        LOG(ERROR) << "when send row data packet, encode packet fails" << endl;
      }
    }
  }
  ...
  //		ret = ProcessSinglePacket(fd, buffer, pos);
  //		if (ret != C_SUCCESS) {
  //			LOG(ERROR)<< "when send row data packet, encode packet
  // fails"
  //			<< endl;
  //		}
  //
...
int CMysqlServer::SendResultSet(int fd, MysqlResultSet& result,
  ...
  if (NULL == &result) {
    ...
    if (C_SUCCESS == ret) {
      LOG(INFO) << "send result set to client " << inet_ntoa(temp.sin_addr)
                << endl;
      number_ = 0;
    }
  }
...
int CMysqlServer::PostPacket(int fd, MysqlSQLPacket* packet, uint8_t seq) {
  ...
  // cout<<"mysql_server.cpp:812: buffer is:"<<bytestohexstring(buffer,50)<<"and
  // the pkt_len is:"<<pkt_len<<endl;
...
int CMysqlServer::ProcessSinglePacket(int fd, char* buffer, int len) {
  ...
  if (len <= 0 || fd < 0 || buffer == NULL) {
    ...
    while (len > 0 && (count = write(fd, buffer, len)) != 0) {
      //			LOG(INFO)<<"this time send "<<count<<"
      // size"<<endl;
      if (count == -1) {
        if (errno == EINTR) {  // if call is interrupted by signal
          continue;
        }
        MySqlElog("%s. write buffer fails, fd is %d, buffer is %p, len is %d.",
                  fd, buffer, len);
        ret = C_ERROR;
        break;
      } else {
  ...
  while (true) {
    ...
    for (i = 2; i < rs.connection_max_number_; i++) {
      if (rs.result_got_[i] == true && rs.sock_fd_[i] > 0) {
        ...
        string buff_to_send;
        httpserver::result_manage(buff_to_send, rs.result_[i]);
        //				cout<<"mysql_server.cpp:865 now result
        // is
        //:"<<buff_to_send<<endl;
        ExecutedResult result_ = rs.result_[i];
        ResultSet* result_set = result_.result_;
        ...
        if (rs.result_[i].status_ != NULL) {
          if (result_set != NULL) {
            ...
            server->SendResponse(rs.sock_fd_[i], &rs.command_packet_[i],
                                 result);
          } else {
            if ("" == rs.result_[i].warning_) {
              ...
              uint16_t server_status = 0;
              server->SendOkPacket(rs.sock_fd_[i], result, server_status,
                                   &rs.command_packet_[i]);
          ...
          uint16_t server_status = 0;
          server->SendErrorPacket(rs.sock_fd_[i], &rs.command_packet_[i],
                                  result);
        }
        ExecutedResult resulttemp;
