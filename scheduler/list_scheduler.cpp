...
ListScheduler::ListScheduler(PipelineJob* const dag_root,
...
void ListScheduler::ScheduleJob(caf::event_based_actor* self,
                                ListScheduler* scheduler) {
  LOG(INFO) << "ListScheduler starts up now!";
  ...
  self->become(
      ...
      caf::others >>
          ...
          [=]() { LOG(WARNING) << "ListScheduler receives unkown message"; });
  self->send(self, SchPJobAtom::value);
}

PipelineJob* ListScheduler::GetPivotJob() {
...
bool ListScheduler::CouldSchedule(PipelineJob* pjob) {
  bool could_sch = true;
...
void ListScheduler::CreateActor() {
