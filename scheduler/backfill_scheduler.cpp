        ...
        if (NULL != pjob) {
            ...
            for (auto mit = pjob->node_task_num_.begin();
              ...
              if (0 != (*it)->node_allocated_thread_[mit->first]) {
                ...
                self->sync_send(Environment::getInstance()
                                    ->get_slave_node()
                                    ->GetNodeActorFromId(mit->first),
                                EpdJobAtom::value, (*it)->get_job_id(),
                                -(*it)->node_allocated_thread_[mit->first])
                    .then(

                        [=](OkAtom) {
                          LOG(INFO) << "query, job id= " << (*it)->get_job_id()
                                    << " has been shrunk!";
                        },
                        caf::others >> [=]() {
                                         LOG(WARNING) << "pipeline shrink job "
                                                         "receives unknown "
                                                         "message";
                                       },
                        caf::after(std::chrono::seconds(kTimeout)) >>
                            [&]() {
                              LOG(WARNING) << "shrink job "
                                           << (*it)->get_job_id()
                                           << " timeout!";
                            }
          ...
          */
          ...
          if (PipelineJob::kReady == pjob->get_job_status()) {
            ...
            // pjob->ExecuteNewJob(self, scheduler);
      ...
      [=](SchEJobAtom) {
        ...
        if (NULL != ejob) {
          if (flag == 0) {
            ...
            // execute the underlying job
            //            ejob->ExecuteNewJob(self, scheduler);
