...
RetCode HdfsLoader::PrepareForLoadFromHdfs() {
  ...
  if (NULL == fs_) ret = rFailure;
  return ret;
}

RetCode HdfsLoader::CloseHdfsFile() { hdfsCloseFile(fs_, file_); }

int HdfsLoader::GetCharFromBuffer(void*& buffer, int& pos, int& read_num,
...
RetCode HdfsLoader::SeekHdfsFile(uint64_t pos) {
  int ret = rSuccess;
  int seekRes = hdfsSeek(fs_, file_, pos);
  if (seekRes != 0) {
    std::cout << "hdfsSeek start fail\n";
    ret = rFailure;
  }
  return ret;
}
