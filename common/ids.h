...
struct PartitionID {
  ...
  PartitionID(ProjectionID projection_id, PartitionOffset off)
      : projection_id(projection_id), partition_off(off), node_id(-1){};
  PartitionID(ProjectionID projection_id, PartitionOffset off, int load_node_id)
  ...
  std::string getName() const {
    ...
    if (node_id == -1) {
      str << "T" << projection_id.table_id << "G"
